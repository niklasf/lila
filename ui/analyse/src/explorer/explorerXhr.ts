import { OpeningData, TablebaseData, ChessdbData } from './interfaces';
...
export function chessdb(fen: Fen): JQueryPromise<ChessdbData> {
  return $.ajax({
    ...
    cache: true
  }).then((data: Partial<ChessdbData>) => {
    data.chessdb = true;
    data.fen = fen;
    return data as ChessdbData;
  });
}
